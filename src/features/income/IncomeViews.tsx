            ...
            <NativeSelect value={partnerId} onValueChange={setPartnerId}>
              ...
              {dashboard.revenuePartners.map((partner) => (
                <NativeSelectOption key={partner.id} value={partner.id}>
                  {partner.name} · {partner.source === "quinstreet" ? `QuinStreet · ${partner.categoryValue}` : "TUNE"} · {partner.teamId ? teamsById.get(partner.teamId)?.name ?? "Unknown owner" : "Company-level"} · {cadenceLabel(partner.billingCadence)}
                </NativeSelectOption>
              ))}
            </NativeSelect>
          </label>
...
export function InvoicesView({
  ...
  function selectInvoiceTab(nextTab: InvoiceTab) {
    setTab(nextTab);
    if (nextTab !== "pending") setStatusFilter("all");
  }
  ...
  return (
    <div className="income-page-stack">
      ...
      <section className="panel invoice-panel">
        ...
        <span className="screen-reader-only" role="status" aria-live="polite">{visibleRows.length} invoices shown in the {tab} view</span>
                        ...
                        <div className="paid-allocation-list">
                          {allocations.length > 0 ? allocations.map((allocation) => (
                            <div className="paid-allocation-entry" key={allocation.id}>
                              ...
                              {paymentAllocationRecordedDifference(allocation) !== null && <small>{money(paymentAllocationRecordedDifference(allocation)!, allocation.currency)} recorded difference <InfoPopover label="payment match difference"><span>{allocation.matchReason}</span><span>The tolerance rule does not verify a bank fee.</span></InfoPopover></small>}
