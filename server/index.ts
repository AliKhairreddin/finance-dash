...
app.use("/api/documents", express.raw({ type: () => true, limit: "10mb" }), async (request, response, next) => {
  try {
    ...
    const result = await handleDocumentApi(new Request(`http://localhost:${port}${request.originalUrl}`, { method: request.method, headers, ...(body ? { body } : {}) }), {
...
app.get("/api/transactions/lookup", async (request, response, next) => {
  try { response.json(await localConvexClient().query(api.banking.getTransaction, { serviceToken: localConvexServiceToken(), id: typeof request.query.id === "string" ? request.query.id : "" })); } catch (error) { next(error); }
});
app.post(
...
function localTransactionPageOptions(request: express.Request): Parameters<typeof getTransactionPage>[0] {
    ...
    const slashVirtualAccountId = typeof request.query.slashVirtualAccountId === "string" ? request.query.slashVirtualAccountId.trim() : "";
    if (slashVirtualAccountId.length > 256) throw new ClientRequestError("Slash virtual account is invalid");
