...
screenshot open pending_invoices slash_report cashback_report share_updates amex
`.trim().split(/\s+/u).sort();
...
test("every command declares whether it is one-tap, optional, or requires details", () => {
  ...
  assert.equal(financeTelegramCommands.filter(({ access, command }) => access === "action" && !["share_updates", "amex"].includes(command)).every(({ input }) => input === "required"), true);
