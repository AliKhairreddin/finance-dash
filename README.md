...
The same report is sent daily at **07:00 Asia/Beirut**, including Beirut's daylight-saving changes, to `TELEGRAM_CASH_REPORT_RECIPIENTS` (Ali and Ali M). The existing minute cron checks the local date/time and retries incomplete deliveries after 07:00. Each recipient has separate durable delivery progress, including progress through a multipart report. This schedule is independent of the optional `/digest` summary. The bot's private command menu includes `/cash` for users who already have reporting access.
