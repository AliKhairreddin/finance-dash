...
The Telegram `/pending_invoices` command lists unpaid sales invoices (including drafts) with outstanding amounts after allocations, in due-date order. `/slash_report` reads exact rolling 24-hour card activity and current virtual-account balances directly from Slash. The same report is delivered daily at 13:00 `Asia/Beirut` only to `TELEGRAM_SLASH_REPORT_RECIPIENTS` (Amin, Ali, and Ali M), with per-recipient retry state separate from the existing weekly cash report. It reports posted spend, pending charges, refunds, and suggested account funding for one further day of gross card activity (three days on Friday to reach Monday) plus the configured `SLASH_VIRTUAL_ACCOUNT_ALERT_THRESHOLD_USD` reserve per open account. Saturday and Sunday reports show projected funding gaps for the remaining two or one days until Monday, and explain that bank transfers resume Monday. Weekdays are evaluated in Beirut time. The `/slash_report` command uses the same recipient restriction. It does not execute transfers or card repayments.
...
Bank transactions are matched to open invoices every five minutes and immediately after manual bank/Merit sync, Merit refresh, and statement import. Exact and fee-tolerant links are saved before the AI pass; AI decisions are checked against current records before saving. A confirmed match that covers the exact outstanding amount automatically records a payment allocation and marks the invoice paid, using the bank's source, account, and payment date. Scheduled reruns do not duplicate payments. Ambiguous and fee-different candidates require review; incomplete candidate history prevents automatic payment recording. Record payment searches all banks, including Wise, Revolut, and Slash, shows the bank name beside each transaction, and preselects a saved match (including older transactions outside the first page). Selecting a transaction fills its bank details; manual payments require an explicit source selection.
...
Ali and Ali M can use `/share_updates` or **Share with partners** on Cash Flow → Position / Open invoices to send both report PNGs to Amin, Sani, Ben, Ali, and Ali M. Sharing uses the most recent saved cash-flow snapshot (save edits first) and current open invoices, including labeled drafts and manual receivables. Website filters do not restrict the report. The existing Sani/Sanjin and Ben/Beno login names resolve to one account per person; missing or ambiguous recipient mappings block the whole request.

Each request creates a `PartnerUpdates` Durable Object, captures the report data once, renders both images using Browser Run and the same portrait cash-flow renderer as the website, then sends identical uncompressed PNG albums to all five people. The website retains the update ID in its URL and shows individual delivery results; the requester also receives a Telegram delivery receipt. Retries reuse the saved images and target only explicit Telegram rejections. Interrupted or timed-out sends are marked unconfirmed and require checking Telegram before requesting another send. The `npm run build` command also builds the standalone `partner-report.js` and `partner-report.css` assets used by Browser Run.

Use `/menu` in Telegram for the role-specific catalog and `/help <command>` for syntax. The native Telegram command list labels every command as `▶ TAP`, `⚙ OPTIONAL`, or `✍ TYPE DETAILS`; selecting a required-detail command without its argument returns its exact syntax and an example without running anything. Responses use compact mobile-friendly sections, human-readable Eastern timestamps, bounded bulleted lists, and no link-preview clutter. Regular transaction lists omit long internal IDs, while `/transaction <ID>` retains the full reference in the dedicated detail view. Responses are protected from forwarding. `/invoice_pdf`, `/expense_document`, and `/export_transactions` deliver the file directly; `/screenshot <page>` uses an authenticated Cloudflare Browser Run capture; and `/open <page>` returns the live authenticated URL. Administrators can manage persistent Slash alert rules and a daily UTC digest with `/alert_add`, `/alert_remove`, `/alert_pause`, `/alert_resume`, `/alert_test`, and `/digest`. All mutation commands require the exact `CONFIRM` marker.

`/cash` shows Wise balances by company/currency, Revolut accounts including Bitcoin and its current approximate USD value, and every open Slash virtual account. Slash parent cash/credit balances are excluded from this report to avoid counting the same funds twice. The report includes currency totals, an approximate combined USD total, bank balance check times in Beirut, and current Coinbase quote timestamps. Stale bank balances and unavailable exchange rates are explicitly labeled; missing quotes never produce a misleading complete USD total. Long reports are split into Telegram messages without dropping accounts.

The same report is sent **every Monday at 07:00 Asia/Beirut**, including Beirut's daylight-saving changes, to `TELEGRAM_CASH_REPORT_RECIPIENTS` (Ali and Ali M). The existing minute cron checks the local weekday/date/time and retries incomplete deliveries after 07:00 on that Monday only. Each recipient has separate durable delivery progress, including progress through a multipart report. This schedule is independent of the optional `/digest` summary. The bot's private command menu includes `/cash` for users who already have reporting access, on any day.
